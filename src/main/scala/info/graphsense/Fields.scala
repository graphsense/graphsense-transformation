package info.graphsense

object Fields {
  val height = "height"
  val timestamp = "timestamp"
  val date = "date"
  val txHash = "txHash"
  val txIndex = "txIndex"
  val txPrefix = "txPrefix"
  val txList = "txList"
  val value = "value"
  val addresses = "addresses"
  val address = "address"
  val addressPrefix = "addressPrefix"
  val totalInput = "totalInput"
  val totalOutput = "totalOutput"
  val n = "n"
  val noIncomingTxs = "noIncomingTxs"
  val noOutgoingTxs = "noOutgoingTxs"
  val firstTx = "firstTx"
  val lastTx = "lastTx"
  val totalReceived = "totalReceived"
  val totalSpent = "totalSpent"
  val noAddresses = "noAddresses"
  val currency = "currency"
  val active = "active"
  val addressId = "addressId"
  val addressIdGroup = "addressIdGroup"
  val addressIdSecondaryGroup = "addressIdSecondaryGroup"
  val coinjoin = "coinJoin"
  val cluster = "cluster"
  val clusterGroup = "clusterGroup"
  val srcAddressId = "srcAddressId"
  val dstAddressId = "dstAddressId"
  val srcAddressIdGroup = "srcAddressIdGroup"
  val srcAddressIdSecondaryGroup = "srcAddressIdSecondaryGroup"
  val dstAddressIdGroup = "dstAddressIdGroup"
  val dstAddressIdSecondaryGroup = "dstAddressIdSecondaryGroup"
  val srcAddressPrefix = "srcAddressPrefix"
  val dstAddressPrefix = "dstAddressPrefix"
  val srcCluster = "srcCluster"
  val dstCluster = "dstCluster"
  val srcClusterGroup = "srcClusterGroup"
  val srcClusterSecondaryGroup = "srcClusterSecondaryGroup"
  val dstClusterGroup = "dstClusterGroup"
  val dstClusterSecondaryGroup = "dstClusterSecondaryGroup"
  val noTransactions = "noTransactions"
  val estimatedValue = "estimatedValue"
  val fiatValues = "fiatValues"
  val srcProperties = "srcProperties"
  val dstProperties = "dstProperties"
  val label = "label"
  val hasLabels = "hasLabels"
  val labelNorm = "labelNorm"
  val labelNormPrefix = "labelNormPrefix"
  val category = "category"
  val abuse = "abuse"
  val hasSrcLabels = "hasSrcLabels"
  val hasDstLabels = "hasDstLabels"
}
